    ...
    set(paramName, value) {
        ...
        if (value !== null && value !== undefined) {
            ...
            if (param.list) {
              ...
              if (param.name == 'InboundShipmentPlanRequestItems') {
                let members = value;
                for (var i = 0; i < members.length; i++) {
                    for (var j in members[i]) {
                        this.values['InboundShipmentPlanRequestItems.member.' + (i + 1) + '.' + j] = members[i][j];
                    }
                }
              } else if (param.name == 'InboundShipmentItems') {
                let members = value;
                for (var i = 0; i < members.length; i++) {
                    for (var j in members[i]) {
                        this.values['InboundShipmentItems.member.' + (i + 1) + '.' + j] = members[i][j];
                    }
                }
              } else {
                let members = value;
